...
async function save() { const values = Object.fromEntries(fields.map((key) => [key, document.querySelector(`#${key}`).checked])); await chrome.storage.sync.set(values); const status=document.querySelector('#saved'); status.textContent='Settings saved.'; setTimeout(()=>status.textContent='',1500); }
